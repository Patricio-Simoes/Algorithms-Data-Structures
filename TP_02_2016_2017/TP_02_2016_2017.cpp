...
void findDirectories(BTREE_NODE* root, int value) {
    ...
    if (root == NULL || value == NULL)
        return;
    // Faz a compãração, no caso do ex. 1000 Kbs
...
int CountAllDirectoryFiles(BTREE_NODE* root, char* name) {
    ...
    if (!IsBtreeNode(root, name))
        return NULL;
    // Procura a diretoria para começar a contagem
    if (IsDirectory(root, name))
        return CountDirectoryFiles(root);
    // Chamada recursiva até encontrar o nó (Diretoria) pretendido
    else
...
bool IsBtreeNode(BTREE_NODE* root, const char* name) {
    ...
    if (IsDirectory(root, name))
        retorno = true;
    // Chamada recursiva até encontrar o nó procurado, (se não encontrar retorna null)
